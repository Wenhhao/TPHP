...
# 1.patient table -------
tbl_ls <- read_excel_allsheets('2025.06.01 西湖样本全部列表 共计33个类型_称量表_edit.xlsx')
tbl_ls$Sheet1 <- NULL
...
nrow(info_tidy) # 985 patietns
12*88+82 == 1138 # samples


# 2.sample table -------
info_smp <- read_excel_allsheets('sample_info_20250714.xlsx') %>% 
  plyr::ldply(.id = 'Batch')
...
rio::export(info_smp, 'TPHP_RCA_sample_info_20250715.xlsx')
